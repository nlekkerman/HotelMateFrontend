...
export default function RoomService({ isAdmin }) {
  ...
  return (
    <div className="container my-4">
          ...
          return (
            <div key={item.id} className="col-md-4 mb-4">
              <div className="card h-100">
                {item.image && <img src={item.image} alt={item.name} className="card-img-top" style={{ objectFit: 'cover', height: '180px' }} />}
                <div className="card-body d-flex flex-column">
                  <h5 className="card-title">{item.name} — €{price.toFixed(2)}</h5>
                  <p className="card-text flex-grow-1">{item.description}</p>
                  <div className="mb-2">
                    <label className="form-label me-2">Qty:</label>
                    <input type="number" min="1" max="99" value={quantities[item.id]} onChange={e => handleQuantityChange(item.id, e.target.value)} className="form-control d-inline-block" style={{ width: '70px' }} disabled={!item.is_on_stock} />
                  </div>
                  <button className="btn btn-primary mt-auto" onClick={() => handleAddToOrder(item)} disabled={!item.is_on_stock}>
                    Add to Order
                  </button>
                </div>
              </div>
      ...
      <button className={`btn position-fixed bottom-0 end-0 m-4 rounded-circle p-3 ${showOrderPanel ? 'bg-light primary-color' : 'btn-primary'}`} onClick={() => setShowOrderPanel(p => !p)}>
        <i className={`bi bi-cart${showOrderPanel ? '-fill' : ''} fs-4`} />
      </button>

      {/* Order Panel */}
      {showOrderPanel && (
        <div className="order-panel position-fixed bottom-0 w-100 bg-white border-top p-3" style={{ maxHeight: '70%', overflowY: 'auto', zIndex: 1000 }}>
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h3 className="mb-0">Active Order</h3>
            <button className="btn-close" onClick={() => setShowOrderPanel(false)} />
          </div>
          ...
          {Object.keys(orderItems).length > 0 ? (
            <ul className="list-group mb-3">
              ...
              <li className="list-group-item d-flex justify-content-between"><em>Tray charge</em><span>€{trayCharge.toFixed(2)}</span></li>
              <li className="list-group-item d-flex justify-content-between fw-bold"><span>Total</span><span>€{total.toFixed(2)}</span></li>
            </ul>
          ) : (
            <p>No active items.</p>
          )}

          {submitError && <div className="alert alert-danger">{String(submitError)}</div>}
          <button className="btn btn-success w-100 mb-4" onClick={handlePlaceOrder} disabled={submitting || !Object.keys(orderItems).length}>
          ...
          <div className="previous-orders">
            ...
            {previousOrders.length === 0 ? (
              ...
              previousOrders.map(ord => (
                <div key={ord.id} className="card mb-2">
                  <div className="card-body">
                    ...
                    <ul className="list-group mt-2">
                      {ord.items.map(oi => (
                        <li key={oi.id} className="list-group-item d-flex justify-content-between">
                          <span>{oi.item.name} × {oi.quantity}</span>
                          <span>€{(Number(oi.item.price) * oi.quantity).toFixed(2)}</span>
                        </li>
                      ))}
                    </ul>
                  </div>
