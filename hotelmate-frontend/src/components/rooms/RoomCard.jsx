...
const RoomCard = ({ room }) => {
  ...
  const handleTurnoverAction = async (actionFn, actionName) => {
    ...
    try {
      ...
      const errorMessage = handleRoomOperationError(
        error,
          ...
          <div className="card-text mb-3">
            ...
            {room.is_bookable !== undefined && (
              <div className="d-flex justify-content-between align-items-center">
                ...
                <span
                  className={`badge ${
                    room.is_bookable ? "bg-success" : "bg-secondary"
                  }`}
                >
              ...
              <small className="text-muted">
                ...
                {room.last_inspected_at && (
                  <>
                    ...
                    Inspected:{" "}
                    {new Date(room.last_inspected_at).toLocaleDateString()}
                    {room.inspected_by_staff &&
                      ` by ${room.inspected_by_staff}`}
                  </>
                )}
              </small>
            </div>
                  ...
                  room.room_status !== "MAINTENANCE_REQUIRED" && (
                    <button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        navigate(
                          `/rooms/${room.hotel_slug}/rooms/${room.room_number}`
                        );
                      }}
                    >
                      <i className="bi bi-tools me-1" />
          ...
          {room.primary_guest && (
            <div className="mb-2">
              <div className="d-flex align-items-center">
                ...
                <strong>
                  {room.primary_guest.first_name} {room.primary_guest.last_name}
                </strong>
              </div>
              {room.companions && room.companions.length > 0 && (
                <small className="text-muted">
                  <i className="bi bi-people me-1" />+{room.companions.length}{" "}
                  companion{room.companions.length > 1 ? "s" : ""}
                </small>
              )}
              {room.walkins && room.walkins.length > 0 && (
                <small className="text-info">
                  <i className="bi bi-door-open me-1" />+{room.walkins.length}{" "}
                  walk-in{room.walkins.length > 1 ? "s" : ""}
                </small>
              )}
            </div>
          )}
